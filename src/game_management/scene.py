import pygame
from src.entities.player_files.player import Player
from src.game_management.camera import Camera
from src.data.map_arrays import *
from src.game_management.map import Map
from src.data.settings import *
from src.UI.UI_screens.pause_menu import PauseMenu

#Base Class
class Scene:
    def __init__(self, game):
        self.game = game

    def update(self):
        pass

    def check_event(self, event):
        pass

    def draw(self):
        pass

#Child Classes
class LevelScene(Scene):
    def __init__(self, game, map, ambient_name):
        super().__init__(game)
        self.map = Map(game, map, game.graphics_lib)
        player_pos = (1.5, 1.5)
        self.player = Player(self, player_pos)
        self.camera = Camera(game, self.player)
        self.pause_menu = PauseMenu(game)
        self.pause_menu.active = False
        self.game.audio_manager.play_music(ambient_name, 0.2)

    def update(self):
        super().update()
        self.player.update()
        self.pause_menu.update()

    def check_event(self, event):
        super().check_event(event)
        self.pause_menu.check_event(event)
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                self.pause_menu.active = not self.pause_menu.active
            self.player.event_update(event)

    def draw(self):
        super().draw()
        self.camera.draw_view()
        self.pause_menu.draw(self.game.screen)
        self.player.draw_ui(self.game.screen)

class UIScene(Scene):
    def __init__(self, game, ui, music_name):
        super().__init__(game)
        self.ui = ui
        self.game.audio_manager.play_music(music_name, 0.3)

    def update(self):
        super().update()
        self.ui.update()

    def check_event(self, event):
        super().check_event(event)
        self.ui.check_event(event)

    def draw(self):
        super().draw()
        self.ui.draw(self.game.screen)